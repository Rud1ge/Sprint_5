...
def test_user_login(driver, existing_user_credentials):
    ...
    # Проверить: после логина отображаются аватар и имя User.
    WebDriverWait(driver, 3).until(expected_conditions.visibility_of_element_located((By.XPATH, BUTTON_PROFILE)))
    assert driver.current_url == "https://qa-desk.education-services.ru/login"
